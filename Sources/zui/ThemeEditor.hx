...
class ThemeEditor {
    ...
    public function new() {
        var font = Assets.fonts.jetbrains;
    ...
    function render(framebuffers: Array<Framebuffer>) {
        ...
        editor.begin(g);
        if(editor.window(Id.handle({selected: true}), 0, 0, 400, System.windowWidth(), false)) {
            ...
            theme.ELEMENT_H = Std.int(editor.slider( Id.handle({ text: ""+theme.ELEMENT_H, value: theme.ELEMENT_H }), "ELEMENT_H", 0, 128, true, 1 ));
            ...
            if(editor.button("Reset")) preview.t = theme = zui.Themes.dark;
        ...
        var htab = Id.handle();
		htab.redraws = 1;
        if(preview.window(htab, 420, 20, 400, 600, true)) {
